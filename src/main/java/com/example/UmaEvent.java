...
public class UmaEvent {
    ...
    private static void processCardEvent(WebDriver driver, String url) {
                        ...
                        if (rows.size() > 0) {
                            for (WebElement row : rows) {
                                ...
                                if (cells.size() >= 2) {
                                    ...
                                    writer.write("    ??????ï¼?" + effectText.toString().trim());
                                    writer.newLine();
                                }
                            }
                        } else {
        ...
        driver.quit();
            ...
            System.out.println("??????äº?ä»¶æ????¼ç????¯èª¤ï¼?" + e.getMessage());
            e.printStackTrace();
        }
    }
    ...
    private static void saveToFile(String cardId, String content) {
        ...
        try (BufferedWriter writer = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(fileName), "UTF-8"))) {
            ...
            System.out.println("å¯«å?¥æ??æ¡??????¯èª¤ï¼?" + e.getMessage());
        }
    }
}
